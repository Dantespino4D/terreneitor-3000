...
void Mi_Espnow::begin(){
    // #1
    nvs_flash_init(); //Almacenamiento no Volatil, pedacito de la memoria flash, abrimos la lectura de NVS
    esp_netif_init(); //Network Interface, sabe manejar direcciones IP, esp no necesita ip ni routers pero esp32 obliga que este despierto antes de encer cualquier antena
    esp_event_loop_create_default(); //Tablero de notificaciones, hace que el circuito fisico avise al programa principal si algo se conecto o le llego algun paquete

    // #2
    wifi_init_config_t configuracionWIFI = WIFI_INIT_CONFIG_DEFAULT(); //Cargamos la configuracion de fabrica  
    esp_wifi_init(&configuracionWIFI); //Le pasamos la configuracion al driver del WI-FI
    esp_wifi_set_mode(WIFI_MODE_STA); // Le decimos que funcione como "Estacion" (Celular buscando Red)
    esp_wifi_start(); // Encendemos la Antena

    // #3
    esp_now_init(); //Iniciamos ESP-NOW
}


void Mi_Espnow::agregarMacAddress(const uint8_t* nuevaMac){
...
void Mi_Espnow::empaquetar(Datos* paqueteListo){
